...
if (!empty($_POST['transit']) && !empty($_POST['institution']) && !empty($_POST['account'])) {
    $stmt = $conn->prepare('INSERT INTO payment_methods (payment_method_user_id) VALUE (:payment_method_user_id)');
    $user_id = $_SESSION["user_id"];
    $stmt->bindParam(':payment_method_user_id', $user_id);

    if ($stmt->execute()) {
        $last_id = $conn->lastInsertId();
        $stmt = $conn->prepare('INSERT INTO paps (pap_payment_method_id, pap_transit_number, pap_institution_number, pap_account_number) VALUES (:pap_payment_method_id, :pap_transit_number, :pap_institution_number, :pap_account_number)');
