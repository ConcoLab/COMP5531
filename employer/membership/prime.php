...
// echo $_SESSION["employer_category"];
// $today = new DateTime(date('Y-m-d'));
// $end_of_month = date('Y-m-t');

// echo $end_of_month - $today;
echo $_SESSION["employer_category"];
if ($_SESSION["employer_category"] != "Gold" && $_SESSION["employer_category"] != "Prime") {
    ...
    if ($result) {
        ...
        if ($stmt->execute()) {
            $stmt_employer = $conn->prepare('UPDATE gxc55311.z_employers SET employer_category = "Prime"
                            WHERE employer_id = :employer_id');
            $stmt_employer->bindParam(':employer_id', $_SESSION['user_id']);
            if ($stmt_employer->execute()) {
                $_SESSION["employer_category"] = "Prime";
                header("Location: .");
            } else {
...
} else if ($_SESSION["employer_category"] == "Gold") {
    $stmt = $conn->prepare('SELECT * FROM gxc55311.z_users
                            WHERE user_id = :user_id LIMIT 1');
    $stmt->bindParam(':user_id', $_SESSION['user_id']);
    $stmt->execute();
    ...
    if ($result) {
        $current_balance = $result['user_balance'];
        echo $current_balance;
        $balance = $current_balance - (50 / 30) * $days_left + (100 / 30) * $days_left;
        echo $balance;
        $stmt_user = $conn->prepare('UPDATE gxc55311.z_users SET user_balance = :user_balance
                            WHERE user_id = :user_id');
        $stmt_user->bindParam(':user_id', $_SESSION['user_id']);
